...
export type ThamSoDeXuat = { workspaceId: string; beMat: BeMat; soLuong: number };
...
export async function deXuatYTuong(thamSo: ThamSoDeXuat): Promise<KetQuaStudio<YTuongDeXuat[]>> {
  ...
  const thamKhao = tinHieu.flatMap((t) => {
    ...
    return [{ maThamKhao: t.id, kieuHook: c.kieuHook, chuDe: c.chuDe, soChu: c.soChu, coCTA: c.coCTA }];
  });
  ...
  const nguonThamKhao = sach.flatMap(({ trendSignalId }, viTri) => {
    if (!trendSignalId) return [];
    const nguon = tinHieu.find((t) => t.id === trendSignalId);
    return [{ viTri, trendSignalId, lienKet: nguon?.lienKet ?? null }];
  });
  ...
  return { ok: true, duLieu: sach.map(({ y, lyDo }) => ({ ...y, lyDoDeXuat: lyDo })), canhBao, nguonThamKhao };
}
